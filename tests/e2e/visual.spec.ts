...
test.describe("@visual visual checks", () => {
	test("homepage navigation desktop snapshot", async ({ page }, testInfo) => {
		test.skip(
			...
			"Desktop visual baseline kept on chromium only",
		);

		await page.goto("/");
		await expect(page.locator("#site-header")).toHaveScreenshot(
			"public-home-header-desktop.png",
		);
	});
	...
	test("homepage navigation mobile snapshot", async ({ page }, testInfo) => {
		test.skip(
			...
			"Mobile visual baseline kept on Mobile Chrome only",
		);

		await page.goto("/");
		await page.getByTestId("nav-burger").click();
		await expect(page.locator("#site-header")).toHaveScreenshot(
			"public-home-header-mobile.png",
		);
	});
	...
	test("koi catalogue top snapshot", async ({ page }, testInfo) => {
		...
		await expect(page.locator(".filter-bar")).toHaveScreenshot(
			"public-kois-filter-desktop.png",
		);
	});
	...
	test("azukari hero snapshot", async ({ page }, testInfo) => {
		...
		await expect(page.locator(".hero-compact")).toHaveScreenshot(
			"public-azukari-hero-desktop.png",
		);
	});
