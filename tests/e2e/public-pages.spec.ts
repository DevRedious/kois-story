...
test.describe("public pages", () => {
	test("@smoke homepage CTA leads to the koi catalogue", async ({ page }) => {
		...
		await expect(
			page.getByRole("heading", {
				name: /Des koïs d'exception, issus de la lignée Konishi/i,
			}),
		).toBeVisible();
	...
	test("@smoke azukari page keeps only the appointment note", async ({
		page,
	}) => {
	...
	test("koi filter bar keeps a stable width while scrolling", async ({
		page,
	}, testInfo) => {
		test.skip(
			testInfo.project.name.includes("Mobile"),
			"Desktop filter bar check only",
		);
	...
	test("public contact form submits successfully", async ({ page }) => {
		...
		await page
			...
			.fill("Bonjour, ceci est un message de verification Playwright.");
		...
		await expect(
			page.getByText(/Votre message a bien ete envoye/i),
		).toBeVisible();
	});
});
