...
test.describe("admin area", () => {
	...
	test("@smoke loads the main admin index pages after login", async ({
		page,
	}) => {
	...
	test("mobile sidebar opens and navigates to catalogue koi", async ({
		page,
	}, testInfo) => {
	...
	test("admin seeded data is visible on key pages", async ({ page }) => {
		...
		await expect(
			page.getByText(/En retard|Pending|En attente/i).first(),
		).toBeVisible();
	...
	test("admin can create a product from the form", async ({
		page,
	}, testInfo) => {
		await loginAsAdmin(page);
		...
		await page
			...
			.fill("Produit cree automatiquement par Playwright.");
