...
test.describe('public navigation', () => {
  test('desktop nav exposes the main entries and about page', async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes('Mobile'), 'Desktop nav check only');
  ...
  test('about page is reachable from the navbar', async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes('Mobile'), 'Desktop nav check only');
    ...
    await expect(page.getByRole('heading', { name: 'En conception' })).toBeVisible();
  });

  test('mobile burger opens the menu and reaches about page', async ({ page }, testInfo) => {
    test.skip(!testInfo.project.name.includes('Mobile'), 'Mobile nav check only');
