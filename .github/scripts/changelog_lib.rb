...
module Changelog
  ...
  def github_repo
    ...
    return repo unless repo.to_s.empty?
  ...
  def github_metadata(ref_sha)
    ...
    commits = {}
    1.upto(20) do |page|
      batch = JSON.parse(run("gh", "api", "repos/#{repo}/commits?sha=#{ref_sha}&per_page=100&page=#{page}"))
    ...
    [commits, prs.to_h { |pr| [pr.fetch("number").to_s, pr.dig("author", "login")] }]
  ...
  def entries(ref)
    ...
    by_date = Hash.new { |hash, date| hash[date] = Hash.new { |inner, category| inner[category] = [] } }
    run("git", "log", "--pretty=format:#{FORMAT}", "--date=short", ref).split("\x1e").each do |record|
      full_sha, short_sha, date, author_name, author_email, subject, body = record.strip.split("\x1f", 7)
      next unless full_sha
      ...
      author = display_subject.match?(/auto-update CHANGELOG/i) ? "Automation" : LOGIN_TO_AUTHOR[login] || LOCAL_AUTHOR_TO_AUTHOR[author_email] || author_name
      by_date[date][category] << { label: label, author: author, sha: short_sha }
    end
    by_date
  end

  def render(entries_by_date)
    output = ["# Changelog", "", "All notable changes to this project are documented in this file.", "",
    ...
    entries_by_date.keys.sort.reverse.each do |date|
      ...
      CATEGORY_ORDER.each do |category|
        rows = entries_by_date[date][category]
        next if rows.empty?

        output << "### #{category}"
        rows.each { |entry| output << "- #{entry[:label]} — #{entry[:author]} (`#{entry[:sha]}`)" }
        output << ""
      end
      output << "---" << ""
    end
